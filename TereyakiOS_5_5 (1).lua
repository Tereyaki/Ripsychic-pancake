...
local TabStats     = Window:AddTab("Info",       "activity")
local TabMain      = Window:AddTab("Farm",       "home")
local TabSkip      = Window:AddTab("Skip List",  "x-circle")
local TabMisc      = Window:AddTab("Misc",       "tool")
...
local GB_Misc_L    = TabMisc:AddLeftGroupbox("Movement")
local GB_Misc_R    = TabMisc:AddRightGroupbox("Doors")
...
do
    ...
    local AF_AC_Distance = 15
    local AF_AC_Cooldown = 0.7
...
do
    ...
    local AL = {
        ...
        Prediction     = false,
        PredictionAmt  = 1,
        BindMode       = "MouseButton2",  -- "MouseButton1" | "MouseButton2" | "Key"
    ...
    -- ── input ──────────────────────────────────────────────────────────────
    -- BindMode: "MouseButton1" | "MouseButton2" | "Key"
    -- При "Key" используется AL.Keybind (по умолчанию E), hold
    local mouse = AL_LP:GetMouse()

    -- Клавиатура (работает только когда BindMode == "Key")
    AL_UIS.InputBegan:Connect(function(key, gameProcessed)
        if gameProcessed then return end
        if AL.BindMode == "Key" and key.KeyCode == AL.Keybind then
    ...
    mouse.Button1Down:Connect(function()
        if AL.BindMode == "MouseButton1" then
            AL.Target       = AL_ClosestToMouse()
            AL.IsAimKeyDown = true
        end
    end)
    ...
    mouse.Button2Down:Connect(function()
        if AL.BindMode == "MouseButton2" then
            AL.Target       = AL_ClosestToMouse()
            AL.IsAimKeyDown = true
        end
    end)
    ...
    -- Один дропдаун для выбора бинда: MB1 / MB2 / Клавиша
    AL_Tab:AddDropdown("AL_BindMode", {
        Text = "Bind (Hold)",
        Values = { "MouseButton1", "MouseButton2", "Key" },
        Default = "MouseButton2", Multi = false,
        Callback = function(v) AL.BindMode = v or "MouseButton2" end,
    })

    -- KeyPicker виден только когда выбран режим "Key"
    AL_Tab:AddLabel("Key (when Bind = Key)"):AddKeyPicker("AL_Keybind", {
...
do
    ...
    local function InfStamina_Enable()
        ...
        InfStamina_Connection = _RunService.RenderStepped:Connect(function()
    ...
    local flySpd  = 60
    local MISC_RUNS = { Rag = nil, Old = nil }
    local fly_hrp = nil

    local evFolderFly = _ReplicatedStorage:FindFirstChild("Events")
    ...
    local function stopRagLoop()
        if MISC_RUNS.Rag then MISC_RUNS.Rag:Disconnect(); MISC_RUNS.Rag = nil end
    end
    local function stopOldFly()
        if MISC_RUNS.Old then MISC_RUNS.Old:Disconnect(); MISC_RUNS.Old = nil end
    end
    local function stopAllFly()
        stopRagLoop(); stopOldFly()
        if fly_hrp then fly_hrp.Velocity = Vector3.new(0,0,0) end
    end

    local function startRagLoop()
        if not ragEv or not fly_hrp then return end
        stopRagLoop()
        MISC_RUNS.Rag = _RunService.RenderStepped:Connect(function()
            if not fly_hrp or not FlySettings.On or FlySettings.Method ~= "Ragdoll" then stopAllFly(); return end
            ragEv:FireServer("__---r", Vector3.new(0,0,0), fly_hrp.CFrame, false)
        end)
    end

    local function enRagFly()
        local c = _LP.Character or _LP.CharacterAdded:Wait()
        fly_hrp = c:FindFirstChild("HumanoidRootPart"); if not fly_hrp then return end
        stopAllFly(); startRagLoop()
        MISC_RUNS.Old = _RunService.RenderStepped:Connect(function()
            if not fly_hrp or not FlySettings.On or FlySettings.Method ~= "Ragdoll" then stopAllFly(); return end
            ...
            fly_hrp.Velocity = mv
            if flyEv then flyEv:FireServer("---r", Vector3.new(0,0,0), fly_hrp.CFrame, false) end
        end)
    end
    ...
    local function enOldFly()
        ...
        MISC_RUNS.Old = _RunService.RenderStepped:Connect(function()
            ...
            if rp then
                ...
                rp.CFrame = rp.CFrame + (md * 50 * _RunService.RenderStepped:Wait())
            end
        end)
    end
    ...
    _LP.CharacterAdded:Connect(function(c)
        ...
        task.wait(0.5); c:WaitForChild("HumanoidRootPart"); task.wait(0.1)
    ...
    local function Fly_Enable()
        ...
        FlySettings.On = true
        if _LP.Character then applyFly() end
    end
    ...
    local function Noclip_Enable()
        ...
        if not Noclip_Connection then
            Noclip_Connection = _RunService.RenderStepped:Connect(function()
    ...
    local function NearbyDoorInteraction_Loop()
        while UnlockNearbyDoors_Enabled do
            local waitTime = 0.25
            local char = _LP.Character
            ...
            local doorsFolder = _Workspace:FindFirstChild("Map") and _Workspace.Map:FindFirstChild("Doors")
            if not doorsFolder then
                warn("[TereyakiOS] AutoUnlockDoors: Workspace.Map.Doors не найдено")
                UnlockNearbyDoors_Enabled = false; break
            end
            ...
            for _, doorInstance in ipairs(doorsFolder:GetChildren()) do
                ...
                if doorBase and valuesFolder and eventsFolder then
                    if (playerPos - doorBase.Position).Magnitude <= checkRadius then
                        ...
                        if lockedValue and lockArgument
                            and typeof(lockedValue.Value) == "boolean"
                            and lockedValue.Value == true
                        then
    ...
    local function UnlockNearbyDoors_Enable()
        ...
        if not NearbyDoorInteraction_Coroutine then
    ...
    -- Левая: Movement
    ML:AddToggle("Misc_InfStamina", {
        Text = "Infinite Stamina", Default = false,
        Callback = function(v)
            if v then InfStamina_Enable() else InfStamina_Disable() end
        end,
    })

    ML:AddToggle("Misc_Fly", {
        Text = "Fly", Default = false,
        Callback = function(v)
            if v then Fly_Enable() else Fly_Disable() end
        end,
    })

    ML:AddDropdown("Misc_FlyMethod", {
        Text = "Fly Method",
        Values = { "Ragdoll", "Old" },
        Default = "Ragdoll", Multi = false,
        Callback = function(v)
            FlySettings.Method = v or "Ragdoll"
            if FlySettings.On then applyFly() end
        end,
    })

    ML:AddSlider("Misc_FlySpeed", {
        Text = "Fly Speed", Default = 60, Min = 10, Max = 300, Rounding = 0, Suffix = " u/s",
        Callback = function(v) flySpd = v end,
    })

    ML:AddToggle("Misc_Noclip", {
    ...
    -- Правая: Doors
    MR:AddToggle("Misc_UnlockDoors", {
        Text = "Auto Unlock Doors", Default = false,
        Callback = function(v)
            if v then UnlockNearbyDoors_Enable() else UnlockNearbyDoors_Disable() end
        end,
    })
    MR:AddLabel("Радиус: 6 студов")
    MR:AddLabel("Авто-снимает замки")
    MR:AddLabel("рядом с персонажем")
end -- Misc block
-- ============================================================
do
...
-- Иконка TereyakiOS через rbxthumb (гарантированно грузит)
pcall(function()
    if game.CoreGui:FindFirstChild("TereyakiOSIcon") then
        game.CoreGui.TereyakiOSIcon:Destroy()
    end
    local sg = Instance.new("ScreenGui")
    sg.Name = "TereyakiOSIcon"
    ...
    -- Ищем заголовок окна Obsidian и вставляем картинку туда
    task.delay(1, function()
        -- Вставляем маленькую иконку рядом с заголовком через ImageLabel в CoreGui
        local icon = Instance.new("ImageLabel")
        icon.Name = "TOS_Icon"
        icon.Image = "rbxthumb://type=Asset&id=105582840285831&w=420&h=420"
