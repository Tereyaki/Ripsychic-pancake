...
local GB_Rage      = TabRage:AddLeftGroupbox("Rage Bot")
local GB_Misc      = TabMisc:AddLeftGroupbox("Misc")
...
local statusLbl    = GB_Stats:AddLabel("Status: Loading...", true, "LBL_Status")
local safesLbl     = GB_Stats:AddLabel("Safes: 0/0",         true, "LBL_Safes")
local registersLbl = GB_Stats:AddLabel("Registers: 0/0",     true, "LBL_Registers")
local remainingLbl = GB_Stats:AddLabel("Remaining: 0/0",     true, "LBL_Remaining")
local suggestionLbl= GB_Stats:AddLabel("Tip: Loading...",    true, "LBL_Tip")
local cashLbl      = GB_Stats:AddLabel("Cash: $0",           true, "LBL_Cash")
local bankLbl      = GB_Stats:AddLabel("Bank: $0",           true, "LBL_Bank")
local allowanceLbl = GB_Stats:AddLabel("Allowance: $0",      true, "LBL_Allowance")

local function setLabel(lbl, key, title, content)
    pcall(function() Options[key]:SetText(title .. ": " .. content) end)
end

task.spawn(function()
    while true do
        if Settings.Enabled then
            setLabel(statusLbl,    "LBL_Status",    "Status",    StatusText)
            setLabel(safesLbl,     "LBL_Safes",     "Safes",     AvailableSafesCount .. "/" .. TotalSafesCount)
            setLabel(registersLbl, "LBL_Registers", "Registers", AvailableRegistersCount .. "/" .. TotalRegistersCount)
            setLabel(remainingLbl, "LBL_Remaining", "Remaining", (AvailableSafesCount + AvailableRegistersCount) .. "/" .. (TotalSafesCount + TotalRegistersCount))
            setLabel(suggestionLbl,"LBL_Tip",       "Tip",       SuggestionText)
        else
            setLabel(statusLbl,    "LBL_Status",    "Status",    "Idle")
            setLabel(safesLbl,     "LBL_Safes",     "Safes",     "0/0")
            setLabel(registersLbl, "LBL_Registers", "Registers", "0/0")
            setLabel(remainingLbl, "LBL_Remaining", "Remaining", "0/0")
            setLabel(suggestionLbl,"LBL_Tip",       "Tip",       "Start the farm")
        end
        pcall(readStatsGui)
        pcall(readCashAmountValue)
        setLabel(cashLbl,      "LBL_Cash",      "Cash",      "$" .. math.floor(StatsInfo.CashAmount))
        setLabel(bankLbl,      "LBL_Bank",      "Bank",      "$" .. math.floor(StatsInfo.BankAmount))
        setLabel(allowanceLbl, "LBL_Allowance", "Allowance", StatsInfo.AllowanceText ~= "" and StatsInfo.AllowanceText or ("$" .. math.floor(StatsInfo.AllowanceAmount)))
        task.wait(0.5)
    end
end)
...
do
    ...
    R_Tab:AddToggle("RB_On", {
        Text = "Rage Bot", Default = false,
        Callback = function(v) if v then RB_Enable() else RB_Disable() end end,
    })

    R_Tab:AddInput("RB_TargetName", {
        Text = "Target Nick (пусто = ближайший)",
        Placeholder = "Введи ник...",
        Finished = false,
        Callback = function(v) RB.TargetName = v or "" end,
    })

    R_Tab:AddSlider("RB_MaxDist", {
        Text = "Max Distance (авто-таргет)", Default = 200, Min = 10, Max = 1000, Rounding = 0, Suffix = " studs",
        Callback = function(v) RB.MaxDist = v end,
    })

    R_Tab:AddSlider("RB_TeleportRange", {
        Text = "Teleport Range (тп если дальше)", Default = 10, Min = 5, Max = 200, Rounding = 0, Suffix = " studs",
        Callback = function(v) RB.TeleportRange = v end,
    })
end
...
do
    ...
    M_Tab:AddToggle("MISC_InfStamina",  { Text = "Infinite Stamina",          Default = false, Callback = function(v) if v then InfStamina_Enable()          else InfStamina_Disable()          end end })
    M_Tab:AddToggle("MISC_UnlockDoors", { Text = "Auto Unlock Doors",         Default = false, Callback = function(v) if v then UnlockNearbyDoors_Enable()   else UnlockNearbyDoors_Disable()   end end })
    M_Tab:AddToggle("MISC_OpenDoors",   { Text = "Auto Open Doors",           Default = false, Callback = function(v) if v then OpenNearbyDoors_Enable()     else OpenNearbyDoors_Disable()     end end })
    M_Tab:AddToggle("MISC_SafeESP",     { Text = "Safe ESP (Ready/Cooldown)", Default = false, Callback = function(v) if v then SafeESP_Enable()             else SafeESP_Disable()             end end })
end
