...
do
    ...
    local AB_Connection = nil
    local AB_isBlocking = false
    local AB_blockCache = {}
    local AB_lastTrigger = 0

    local function AB_getRemotes()
        local evFolder  = _RS:FindFirstChild("Events")
        local valFolder = _RS:FindFirstChild("Values")
        local mRemote   = evFolder  and evFolder:FindFirstChild("XMHH.2")
        local serverTick= valFolder and valFolder:FindFirstChild("ServerTick")
        return mRemote, serverTick
    end

    local function AB_Enable()
        if AB_Enabled then return end
        AB_Enabled    = true
        AB_isBlocking = false
        AB_blockCache = {}

        AB_Connection = _RunService.Heartbeat:Connect(function()
            if not AB_Enabled then return end
            local mRemote, serverTick = AB_getRemotes()
            if not mRemote or not serverTick then return end
            ...
            if AB_blockCache[tool] == nil then
                local can = false
                pcall(function()
                ...
                AB_blockCache[tool] = can
            end

            if not AB_blockCache[tool] then
                if AB_isBlocking then
                    pcall(function()
                        mRemote:InvokeServer("🍞", math.floor(serverTick.Value - 81919), tool, "BZLZSTI2")
                    end)
                ...
                if p ~= _LP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if (eHrp.Position - hrp.Position).Magnitude <= 15 then
                        ...
                        if eTool and not (eTool:GetAttribute("Unblockable") or eTool:GetAttribute("IgnoreBlock")) then
                            ...
                            if v then
                                if (v:FindFirstChild("Slashing1") and v.Slashing1.Value)
                                or (v:FindFirstChild("Attacking") and v.Attacking.Value) then
                                    attacking = true
                                else
                                    for _, val in ipairs(v:GetChildren()) do
                                        if val.Name:find("Slashing") and val.Value == true then
                            ...
                            if not attacking then
                                ...
                                if h then
                                    for _, t in ipairs(h:GetPlayingAnimationTracks()) do
                                        ...
                                        if n:find("slash") or n:find("swing") or n:find("attack")
                                        or n:find("lunge") or n:find("heavy") then
                                            attacking = true; break
                                        end
                                    end
                                end
            ...
            if shouldBlock then
                AB_lastTrigger = tick()
                if not AB_isBlocking then
                    AB_isBlocking = true
                    task.spawn(function()
                        while AB_isBlocking and char and tool.Parent == char do
                            pcall(function()
                                mRemote:InvokeServer("🍞", math.floor(serverTick.Value - 81919), tool, "BLSTAX1")
                            end)
                            task.wait(0.1)
                            if tick() - AB_lastTrigger > 0.2 then break end
                        end
                        AB_isBlocking = false
                    end)
                end
            elseif AB_isBlocking and tick() - AB_lastTrigger > 0.2 then
                AB_isBlocking = false
                pcall(function()
                    mRemote:InvokeServer("🍞", math.floor(serverTick.Value - 81919), tool, "BZLZSTI2")
                end)
    ...
    local function AB_Disable()
        AB_Enabled = false
        if AB_Connection then
            AB_Connection:Disconnect()
            AB_Connection = nil
        end
        AB_isBlocking = false
        AB_blockCache = {}
    ...
    --  AUTO UNLOCK DOORS
    -- ══════════════════════════════════════════════════════
    local AUD_Enabled   = false
    local AUD_Coroutine = nil

    local function AUD_Loop()
        while AUD_Enabled do
            local char = _LP.Character
            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            ...
            for _, doorInstance in ipairs(doorsFolder:GetChildren()) do
                ...
                if doorBase and valuesFolder and eventsFolder then
                    if (playerPos - doorBase.Position).Magnitude <= 6 then
                        ...
                        if toggleEvent and lockedValue and lockArgument
                        and typeof(lockedValue.Value) == "boolean"
                        and lockedValue.Value == true then
                            pcall(function()
                                toggleEvent:FireServer("Unlock", lockArgument)
                            end)
                        end
                    end
                end
    ...
    local function AUD_Enable()
        if AUD_Enabled then return end
        AUD_Enabled   = true
        AUD_Coroutine = task.spawn(AUD_Loop)
    end

    local function AUD_Disable()
        AUD_Enabled = false
    end
    ...
    --  UI
    -- ══════════════════════════════════════════════════════
    GB_Misc_L:AddToggle("Misc_InfStamina", {
    ...
    GB_Misc_L:AddToggle("Misc_AutoBlock", {
        Text    = "Auto Block",
        Default = false,
        Callback = function(v)
            if v then AB_Enable() else AB_Disable() end
        end,
    })

    GB_Misc_L:AddToggle("Misc_AutoUnlockDoors", {
        Text    = "Auto Unlock Doors",
        Default = false,
        Callback = function(v)
            if v then AUD_Enable() else AUD_Disable() end
        end,
    })
end -- Misc block
-- ============================================================
