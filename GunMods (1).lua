...
local function safeLoad(url, name)
    ...
    if not ok or not src then error("[GunMods] HttpGet failed: " .. name, 2) end
    local fn, err = loadstring(src)
    if not fn then error("[GunMods] loadstring " .. name .. " → " .. tostring(err), 2) end
    local ok2, result = pcall(fn)
    if not ok2 then error("[GunMods] run " .. name .. " → " .. tostring(result), 2) end
    return result
end
...
local TabGuns     = Window:AddTab('Guns',     'crosshair')
local TabBullets  = Window:AddTab('Bullets',  'zap')
local TabTracers  = Window:AddTab('Tracers',  'eye')
local TabHitman   = Window:AddTab('Hitman',   'target')
local TabUISet    = Window:AddTab('UI',       'settings')

local GB_Fire     = TabGuns:AddLeftGroupbox('Fire')
local GB_Gun      = TabGuns:AddRightGroupbox('Gun')
local GB_Bullet   = TabBullets:AddLeftGroupbox('Projectile')
local GB_Reload   = TabBullets:AddRightGroupbox('Reload')
local GB_TracersL = TabTracers:AddLeftGroupbox('Self')
local GB_TracersR = TabTracers:AddRightGroupbox('Others')
local GB_HitmanL  = TabHitman:AddLeftGroupbox('Hitman Briefcase')
local GB_HitmanR  = TabHitman:AddRightGroupbox('Options')
local GB_UISet    = TabUISet:AddLeftGroupbox('Menu', 'wrench')
local GB_UITheme  = TabUISet:AddRightGroupbox('Theme')

-- ── Guns: Fire ──────────────────────────────────────────────
GB_Fire:AddToggle('gm_rapid_fire', {
    Text = 'Rapid Fire', Default = Settings.RapidFire,
    Callback = function(v) Settings.RapidFire = v; onConfigChanged() end,
})
GB_Fire:AddToggle('gm_no_slowdown', {
    Text = 'No Slowdown', Default = Settings.NoSlowdown,
    Callback = function(v) Settings.NoSlowdown = v; onConfigChanged() end,
})
GB_Fire:AddToggle('gm_instant_charge', {
    Text = 'Instant Charge', Default = Settings.InstantCharge,
    Callback = function(v) Settings.InstantCharge = v; onConfigChanged() end,
})
GB_Fire:AddToggle('gm_infinite_range', {
    Text = 'Infinite Range', Default = Settings.InfiniteRange,
    Callback = function(v) Settings.InfiniteRange = v; onConfigChanged() end,
})
GB_Fire:AddToggle('gm_mod_recoil', {
    Text = 'Modify Recoil', Default = Settings.ModRecoil,
    Callback = function(v) Settings.ModRecoil = v; onConfigChanged() end,
})
GB_Fire:AddSlider('gm_recoil_amount', {
    Text = 'Recoil Amount', Default = Settings.RecoilAmount,
    Min = 0, Max = 100, Rounding = 0, Suffix = ' %',
    Callback = function(v) Settings.RecoilAmount = v; onConfigChanged() end,
})
GB_Fire:AddToggle('gm_mod_spread', {
    Text = 'Modify Spread', Default = Settings.ModSpread,
    Callback = function(v) Settings.ModSpread = v; onConfigChanged() end,
})
GB_Fire:AddSlider('gm_spread_amount', {
    Text = 'Spread Amount', Default = Settings.SpreadAmount,
    Min = 0, Max = 100, Rounding = 0, Suffix = ' %',
    Callback = function(v) Settings.SpreadAmount = v; onConfigChanged() end,
})

-- ── Guns: Gun ──────────────────────────────────────────────
GB_Gun:AddToggle('gm_mod_equip', {
    Text = 'Equip Speed', Default = Settings.ModEquip,
    Callback = function(v) Settings.ModEquip = v; onConfigChanged() end,
})
GB_Gun:AddSlider('gm_equip_speed', {
    Text = 'Equip Multiplier', Default = Settings.EquipSpeed,
    Min = 0, Max = 5, Rounding = 2, Suffix = 'x',
    Callback = function(v) Settings.EquipSpeed = v; onConfigChanged() end,
})
GB_Gun:AddToggle('gm_mod_aim', {
    Text = 'Aim Speed', Default = Settings.ModAim,
    Callback = function(v) Settings.ModAim = v; onConfigChanged() end,
})
GB_Gun:AddSlider('gm_aim_speed', {
    Text = 'Aim Multiplier', Default = Settings.AimSpeed,
    Min = 0, Max = 5, Rounding = 2, Suffix = 'x',
    Callback = function(v) Settings.AimSpeed = v; onConfigChanged() end,
})

-- ── Bullets ─────────────────────────────────────────────────
GB_Bullet:AddToggle('gm_instant', {
    Text = 'Instant Bullet', Default = Settings.InstantBullet,
    Callback = function(v)
        Settings.InstantBullet = v
        installProjectileHook()
        ensureProjectileHookRetryLoop()
        hookProjectileHandlersFromConnections()
        onConfigChanged()
...
GB_Bullet:AddSlider('gm_instant_velocity', {
    Text = 'Instant Velocity', Default = Settings.InstantVelocity,
    Min = 1000, Max = 5000000, Rounding = 0, Suffix = ' u/s',
    Callback = function(v) Settings.InstantVelocity = v; onConfigChanged() end,
})
GB_Bullet:AddSlider('gm_instant_range', {
    Text = 'Instant Range', Default = Settings.InstantRange,
    Min = 1000, Max = 500000, Rounding = 0, Suffix = ' st',
    Callback = function(v) Settings.InstantRange = v; onConfigChanged() end,
})

GB_Reload:AddToggle('gm_instant_reload', {
    Text = 'Instant Reload', Default = Settings.InstantReload,
    Callback = function(v)
        Settings.InstantReload = v
        if v then task.spawn(handleInstantReloadFromSource) end
    end,
})

-- ── Tracers: Self ───────────────────────────────────────────
local SelfTracerToggle = GB_TracersL:AddToggle('gm_tracers_self', {
    Text = 'Self Tracers', Default = Settings.TracersSelf,
    Callback = function(v)
        Settings.TracersSelf = v
        if v or Settings.TracersOthers then
            installVisualizeTracerHook()
            installLocalShootTracerHook()
        end
    end,
})
SelfTracerToggle:AddColorPicker('gm_tracer_self_color', {
    Default = Settings.TracerSelfColor, Title = 'Self Tracer Color',
    Callback = function(v) Settings.TracerSelfColor = v end,
})

-- ── Tracers: Others ─────────────────────────────────────────
local OthersTracerToggle = GB_TracersR:AddToggle('gm_tracers_others', {
    Text = 'Players Tracers', Default = Settings.TracersOthers,
    Callback = function(v)
        Settings.TracersOthers = v
        if v or Settings.TracersSelf then
            installVisualizeTracerHook()
            installLocalShootTracerHook()
        end
    end,
})
OthersTracerToggle:AddColorPicker('gm_tracer_others_color', {
    Default = Settings.TracerOthersColor, Title = 'Others Tracer Color',
    Callback = function(v) Settings.TracerOthersColor = v end,
})

-- ── Hitman Briefcase ────────────────────────────────────────
GB_HitmanL:AddToggle('gm_hitman_briefcase', {
    Text = 'Hitman Briefcase', Default = Settings.HitmanBriefcaseEnabled,
    Callback = function(v)
        Settings.HitmanBriefcaseEnabled = v
        syncHitmanWallbangState(shouldEnableHitmanWallbang())
        if not v and type(hitmanClearProjectiles) == 'function' then
            hitmanClearProjectiles()
        end
    end,
})
GB_HitmanL:AddToggle('gm_hb_phase', {
    Text = 'Phase Through World', Default = Settings.HitmanBriefcasePhaseThroughWorld,
    Callback = function(v) Settings.HitmanBriefcasePhaseThroughWorld = v end,
})
GB_HitmanL:AddToggle('gm_hb_pathing', {
    Text = 'Obstacle Pathing', Default = Settings.HitmanBriefcaseObstaclePathing,
    Callback = function(v) Settings.HitmanBriefcaseObstaclePathing = v end,
})
GB_HitmanL:AddToggle('gm_hb_air', {
    Text = 'Air Detour', Default = Settings.HitmanBriefcaseAirDetourEnabled,
    Callback = function(v) Settings.HitmanBriefcaseAirDetourEnabled = v end,
})
GB_HitmanL:AddToggle('gm_hb_quick', {
    Text = 'Quick Detour', Default = Settings.HitmanBriefcaseQuickDetourEnabled,
    Callback = function(v) Settings.HitmanBriefcaseQuickDetourEnabled = v end,
})

GB_HitmanR:AddToggle('gm_hb_emergency', {
    Text = 'Emergency Wall Bypass', Default = Settings.HitmanBriefcaseEmergencyWallBypass,
    Callback = function(v) Settings.HitmanBriefcaseEmergencyWallBypass = v end,
})
GB_HitmanR:AddToggle('gm_hb_lifeextend', {
    Text = 'Life Extend On Close', Default = Settings.HitmanBriefcaseLifeExtendOnClose,
    Callback = function(v) Settings.HitmanBriefcaseLifeExtendOnClose = v end,
})
GB_HitmanR:AddToggle('gm_hb_endgame', {
    Text = 'Endgame Boost', Default = Settings.HitmanBriefcaseEndgameBoostEnabled,
    Callback = function(v) Settings.HitmanBriefcaseEndgameBoostEnabled = v end,
})
GB_HitmanR:AddToggle('gm_hb_preemptive', {
    Text = 'Preemptive Avoidance', Default = Settings.HitmanBriefcasePreemptiveAvoidanceEnabled,
    Callback = function(v) Settings.HitmanBriefcasePreemptiveAvoidanceEnabled = v end,
})
GB_HitmanR:AddToggle('gm_hb_rescue', {
    Text = 'Rescue Steering', Default = Settings.HitmanBriefcaseRescueSteeringEnabled,
    Callback = function(v) Settings.HitmanBriefcaseRescueSteeringEnabled = v end,
})
GB_HitmanR:AddToggle('gm_hb_terrain', {
    Text = 'Terrain Adaptive', Default = Settings.HitmanBriefcaseTerrainAdaptiveEnabled,
    Callback = function(v) Settings.HitmanBriefcaseTerrainAdaptiveEnabled = v end,
})
GB_HitmanR:AddToggle('gm_hb_clutter', {
    Text = 'Clutter Adaptive', Default = Settings.HitmanBriefcaseClutterAdaptiveEnabled,
    Callback = function(v) Settings.HitmanBriefcaseClutterAdaptiveEnabled = v end,
})
...
local refreshConnection = RunService.Heartbeat:Connect(function(dt)
    ...
    if refreshTick < 0.1 then return end
    refreshTick = 0

    local shouldRefreshConfigs = Settings.InstantBullet or Settings.ModRecoil
        or Settings.ModSpread or Settings.ModEquip or Settings.ModAim
        or Settings.InfiniteRange or Settings.NoSlowdown
        or Settings.InstantCharge or Settings.RapidFire
    ...
    if Settings.TracersSelf or Settings.TracersOthers then
        if not visualizeTracerHooked then installVisualizeTracerHook() end
        if not localShootTracerHooked then installLocalShootTracerHook() end
    end
    ...
    if shouldEnableHitmanWallbang() then
        ...
        hitmanWallbangRefreshTick = 0
        if hitmanWallbangApplied then syncHitmanWallbangState(false) end
    end
end)

-- ── UI Settings ─────────────────────────────────────────────
GB_UISet:AddLabel('Menu bind'):AddKeyPicker('MenuKeybind', {
    Default = 'End', NoUI = true, Text = 'Menu keybind',
})
GB_UISet:AddButton({ Text = 'Unload', Func = function() Library:Unload() end })
...
ThemeManager:SetFolder('GunMods')
SaveManager:SetFolder('GunMods/configs')
SaveManager:BuildConfigSection(GB_UISet)
...
Library:OnUnload(function()
    ...
    for _, c in ipairs(visualizeConnections) do pcall(function() c:Disconnect() end) end

    if tracerCameraChangedConnection then
        pcall(function() tracerCameraChangedConnection:Disconnect() end)
        tracerCameraChangedConnection = nil
    end
    if tracerCameraChildAddedConnection then
        pcall(function() tracerCameraChildAddedConnection:Disconnect() end)
        tracerCameraChildAddedConnection = nil
    end
    if tracerBulletAddedConnection then
        pcall(function() tracerBulletAddedConnection:Disconnect() end)
        tracerBulletAddedConnection = nil
    end
    if charAddedConnection      then charAddedConnection:Disconnect()      end
    if charRemovedConnection    then charRemovedConnection:Disconnect()    end
    if backpackAddedConnection  then backpackAddedConnection:Disconnect()  end
    if characterAddedConnection then characterAddedConnection:Disconnect() end
    if hitmanPlayerAddedConnection   then hitmanPlayerAddedConnection:Disconnect()   end
    if hitmanPlayerRemovingConnection then hitmanPlayerRemovingConnection:Disconnect() end
    if hitmanDebrisSpitballConnection then hitmanDebrisSpitballConnection:Disconnect() end

    Settings.InstantReload = false
    Settings.HitmanBriefcaseEnabled = false
    syncHitmanWallbangState(false)
    if type(hitmanClearProjectiles) == 'function' then hitmanClearProjectiles() end
