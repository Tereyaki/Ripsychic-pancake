...
local TabRage      = Window:AddTab("Rage",        "flame")
local TabMisc      = Window:AddTab("Misc",        "settings")
local TabUISet     = Window:AddTab("UI",          "sliders")

-- Groupbox'ы (Obsidian: Left/Right)
local GB = {
    Main      = TabMain:AddLeftGroupbox("Farm"),
    Stats     = TabStats:AddLeftGroupbox("Info"),
    Skip      = TabSkip:AddLeftGroupbox("Skip List"),
    AltFarm   = TabAltFarm:AddLeftGroupbox("AltFarm"),
    Aimlock   = TabAimlock:AddLeftGroupbox("Aimlock"),
    SilentAim = TabSilentAim:AddLeftGroupbox("Silent Aim"),
    SilentAimR= TabSilentAim:AddRightGroupbox("Hit Chances"),
    ESP       = TabESP:AddLeftGroupbox("ESP"),
    ESPR      = TabESP:AddRightGroupbox("ESP Options"),
    Rage      = TabRage:AddLeftGroupbox("Rage Bot"),
    Misc      = TabMisc:AddLeftGroupbox("Misc"),
}

-- Прокси-таблица Tabs — методы перенаправляем на GB
-- (весь старый код делает Tabs.X:CreateToggle → теперь GB.X:AddToggle)
local function makeTabProxy(gb)
    return setmetatable({}, {
        __index = function(_, method)
            -- Маппинг Rayfield → Obsidian методов
            local map = {
                ...
                CreateKeybind     = "AddKeyPicker",
                CreateButton      = "AddButton",
                CreateParagraph   = "AddLabel",
            }
            local obsMethod = map[method]
            if not obsMethod then return function() end end

            return function(_, rayfieldOpts)
                -- Конвертируем Rayfield-опции в Obsidian-опции
                local idx = rayfieldOpts.Flag or rayfieldOpts.Name or tostring(math.random(1,99999))
                local cb  = rayfieldOpts.Callback or function() end

                if method == "CreateToggle" then
                    gb:AddToggle(idx, {
                        Text    = rayfieldOpts.Name,
                        Default = rayfieldOpts.CurrentValue or false,
                        Callback = cb,
                    })
                    return setmetatable({}, { __index = function(_, m)
                        if m == "Set" then return function(_, v) if Toggles[idx] then Toggles[idx]:SetValue(v) end end end
                        return function() end
                    end})

                elseif method == "CreateSlider" then
                    local range = rayfieldOpts.Range or {0, 100}
                    gb:AddSlider(idx, {
                        Text    = rayfieldOpts.Name,
                        Default = rayfieldOpts.CurrentValue or range[1],
                        Min     = range[1],
                        Max     = range[2],
                        Rounding = 0,
                        Suffix  = rayfieldOpts.Suffix or "",
                        Callback = cb,
                    })

                elseif method == "CreateDropdown" then
                    local opts = rayfieldOpts.Options or {}
                    local cur  = rayfieldOpts.CurrentOption
                    local def  = (type(cur) == "table" and cur[1]) or cur or opts[1]
                    gb:AddDropdown(idx, {
                        Text   = rayfieldOpts.Name,
                        Values = opts,
                        Default = def,
                        Multi  = rayfieldOpts.MultipleOptions or false,
                        Callback = function(v)
                            if rayfieldOpts.MultipleOptions then cb(v)
                            else cb(v) end
                        end,
                    })

                elseif method == "CreateInput" then
                    gb:AddInput(idx, {
                        Text        = rayfieldOpts.Name,
                        Default     = rayfieldOpts.CurrentValue or "",
                        Placeholder = rayfieldOpts.PlaceholderText or "",
                        Finished    = rayfieldOpts.RemoveTextAfterFocusLost == false and false or true,
                        Callback    = cb,
                    })

                elseif method == "CreateColorPicker" then
                    gb:AddLabel(rayfieldOpts.Name):AddColorPicker(idx, {
                        Default  = rayfieldOpts.Color or Color3.new(1,1,1),
                        Callback = cb,
                    })

                elseif method == "CreateKeybind" then
                    gb:AddLabel(rayfieldOpts.Name):AddKeyPicker(idx, {
                        Default  = rayfieldOpts.CurrentKeybind or "E",
                        Mode     = "Hold",
                        NoUI     = false,
                        Text     = rayfieldOpts.Name,
                        Callback = cb,
                ...
                elseif method == "CreateButton" then
                    gb:AddButton({ Text = rayfieldOpts.Name, Func = cb })

                elseif method == "CreateParagraph" then
                    -- Возвращаем объект с методом Set
                    local lbl = gb:AddLabel(
                        (rayfieldOpts.Title or "") .. ": " .. (rayfieldOpts.Content or ""),
                        true,
                        idx
                    )
                    return setmetatable({}, { __index = function(_, m)
                        if m == "Set" then
                            return function(_, t)
                                pcall(function()
                                    Options[idx]:SetText((t.Title or "") .. ": " .. (t.Content or ""))
                                end)
                            end
                        end
                        return function() end
...
local Tabs = {
    Main      = makeTabProxy(GB.Main),
    Stats     = makeTabProxy(GB.Stats),
    Skip      = makeTabProxy(GB.Skip),
    AltFarm   = makeTabProxy(GB.AltFarm),
    Aimlock   = makeTabProxy(GB.Aimlock),
    SilentAim = makeTabProxy(GB.SilentAim),
    ESP       = makeTabProxy(GB.ESP),
    Rage      = makeTabProxy(GB.Rage),
    Misc      = makeTabProxy(GB.Misc),
}
...
do
    ...
    AL_Tab:CreateDropdown({
        ...
        Callback      = function(v)
            if type(v) == "table" then v = v[1] end
            AL.MouseBind = v or "MouseButton2"
        end,
    })

    AL_Tab:CreateKeybind({
        Name         = "Key Bind (when not using mouse)",
        CurrentKeybind = "E",
        HoldToInteract = false,
        Flag         = "AL_Keybind",
        Callback     = function(v)
            AL.Keybind = Enum.KeyCode[v] or Enum.KeyCode.E
        end,
    })
end
...
do
    local SA_Tab = Tabs.SilentAim

    SA_Tab:CreateToggle({ Name = "Silent Aim", CurrentValue = false, Flag = "SA_On",
        Callback = function(v) if v then SA_Enable() else SA_Disable() end end })

    SA_Tab:CreateToggle({ Name = "Show FOV Circle", CurrentValue = false, Flag = "SA_DrawFOV",
        Callback = function(v) SA.DrawCircle = v end })

    SA_Tab:CreateSlider({ Name = "FOV Radius", Range = {50,500}, Increment = 1,
        Suffix = "px", CurrentValue = 150, Flag = "SA_FOVSize",
        Callback = function(v) SA.DrawSize = v end })

    SA_Tab:CreateSlider({ Name = "Max Distance", Range = {50,2000}, Increment = 10,
        Suffix = " studs", CurrentValue = 500, Flag = "SA_MaxDist",
        Callback = function(v) SA.MaxDist = v end })

    SA_Tab:CreateToggle({ Name = "Use FOV", CurrentValue = true, Flag = "SA_UseFOV",
        Callback = function(v) SA.UseFOV = v end })

    SA_Tab:CreateToggle({ Name = "Visibility Check (Wall Check)", CurrentValue = true, Flag = "SA_VisCheck",
        Callback = function(v) SA.VisCheck = v end })

    SA_Tab:CreateToggle({ Name = "Ignore Downed Players", CurrentValue = true, Flag = "SA_IgnoreDowned",
        Callback = function(v) SA.IgnoreDowned = v end })
    ...
    for _, entry in ipairs(partDefs) do
        local flag = "SA_HC_" .. entry.key
        SA_Tab:CreateSlider({ Name = entry.label .. " Hit Chance", Range = {0,100}, Increment = 1,
            Suffix = "%", CurrentValue = entry.default, Flag = flag,
...
do
    ...
    -- Основные тогглы
    ESP_Tab:CreateToggle({ Name = "Enable ESP", CurrentValue = false, Flag = "ESP_Enable",
        Callback = function(v) ESP_Settings.Enabled = v end })

    ESP_Tab:CreateToggle({ Name = "Team Check", CurrentValue = false, Flag = "ESP_TeamCheck",
        Callback = function(v) ESP_Settings.TeamCheck = v end })

    ESP_Tab:CreateSlider({ Name = "Max Distance", Range = {100, 4000}, Increment = 50,
        Suffix = " studs", CurrentValue = 4000, Flag = "ESP_MaxDist",
        Callback = function(v) ESP_Settings.MaxDistance = v end })

    -- Box
    ESP_Tab:CreateToggle({ Name = "Box", CurrentValue = false, Flag = "ESP_Box",
        Callback = function(v) ESP_Settings.Box.Box = v end })

    ESP_Tab:CreateToggle({ Name = "Box Outline", CurrentValue = false, Flag = "ESP_BoxOutline",
        Callback = function(v) ESP_Settings.Box.Outline = v end })

    ESP_Tab:CreateColorPicker({ Name = "Box Color", Color = Color3.fromRGB(255,255,255), Flag = "ESP_BoxColor",
        Callback = function(v) ESP_Settings.Box.Color = v end })

    ESP_Tab:CreateColorPicker({ Name = "Outline Color", Color = Color3.fromRGB(0,0,0), Flag = "ESP_OutlineColor",
        Callback = function(v) ESP_Settings.Box.OutlineColor = v; ESP_Settings.Tracer.OutlineColor = v end })

    -- Инфо
    ESP_Tab:CreateToggle({ Name = "Name", CurrentValue = false, Flag = "ESP_Name",
        Callback = function(v) ESP_Settings.Box.Name = v end })

    ESP_Tab:CreateToggle({ Name = "Distance", CurrentValue = false, Flag = "ESP_Distance",
        Callback = function(v) ESP_Settings.Box.Distance = v end })

    ESP_Tab:CreateToggle({ Name = "Health Text", CurrentValue = false, Flag = "ESP_HealthText",
        Callback = function(v) ESP_Settings.Box.Health = v end })

    ESP_Tab:CreateToggle({ Name = "Health Bar", CurrentValue = false, Flag = "ESP_HealthBar",
        Callback = function(v) ESP_Settings.Box.HealthBar = v end })

    -- Tracer
    ESP_Tab:CreateToggle({ Name = "Tracer", CurrentValue = false, Flag = "ESP_Tracer",
        Callback = function(v) ESP_Settings.Tracer.Tracer = v end })

    ESP_Tab:CreateToggle({ Name = "Tracer Outline", CurrentValue = false, Flag = "ESP_TracerOutline",
        Callback = function(v) ESP_Settings.Tracer.Outline = v end })

    ESP_Tab:CreateColorPicker({ Name = "Tracer Color", Color = Color3.fromRGB(255,255,255), Flag = "ESP_TracerColor",
        Callback = function(v) ESP_Settings.Tracer.Color = v end })

    -- Highlight
    ESP_Tab:CreateToggle({ Name = "Highlight", CurrentValue = false, Flag = "ESP_Hilight",
        Callback = function(v) ESP_Settings.Hilights.Hilights = v end })

    ESP_Tab:CreateToggle({ Name = "Highlight Through Walls", CurrentValue = false, Flag = "ESP_HilightWalls",
        Callback = function(v) ESP_Settings.Hilights.AllWaysVisible = v end })

    ESP_Tab:CreateColorPicker({ Name = "Highlight Outline Color", Color = Color3.fromRGB(255,0,0), Flag = "ESP_HilightOutlineColor",
        Callback = function(v) ESP_Settings.Hilights.OutlineColor = v end })

    ESP_Tab:CreateColorPicker({ Name = "Highlight Fill Color", Color = Color3.fromRGB(255,255,255), Flag = "ESP_HilightFillColor",
        Callback = function(v) ESP_Settings.Hilights.FillColor = v end })

    ESP_Tab:CreateSlider({ Name = "Highlight Outline Transparency", Range = {0,100}, Increment = 1,
        Suffix = "%", CurrentValue = 50, Flag = "ESP_HilightOutlineTransp",
        Callback = function(v) ESP_Settings.Hilights.OutlineTransparency = v / 100 end })

    ESP_Tab:CreateSlider({ Name = "Highlight Fill Transparency", Range = {0,100}, Increment = 1,
        Suffix = "%", CurrentValue = 50, Flag = "ESP_HilightFillTransp",
        Callback = function(v) ESP_Settings.Hilights.FillTransparency = v / 100 end })
end

-- ============================================================
--  RAGE BOT  (из бабушкасвэг, улучшен: таргет по нику + телепорт + локинг пуль)
-- ============================================================

local RB = {
    Enabled       = false,
    TargetName    = "",        -- ник конкретного таргета (пусто = ближайший)
    FireRate      = 0,         -- максимальная скорость (как в бабушкасвэг)
    MaxDist       = 200,       -- макс. дистанция для автовыбора ближайшего
    TeleportRange = 10,        -- телепортируемся если дальше этого числа стадов
}

-- Кэшируем ремоты (их ищем один раз, потом переиспользуем)
local RB_EventsFolder = ReplicatedStorage:FindFirstChild("Events")
local RB_GNX    = RB_EventsFolder and RB_EventsFolder:FindFirstChild("GNX_S")
local RB_ZFKLF  = RB_EventsFolder and RB_EventsFolder:FindFirstChild("ZFKLF__H")

-- Пересканируем ремоты если не нашли сразу
local function RB_EnsureRemotes()
    if RB_GNX and RB_ZFKLF then return true end
    RB_EventsFolder = ReplicatedStorage:FindFirstChild("Events")
    if not RB_EventsFolder then return false end
    RB_GNX   = RB_EventsFolder:FindFirstChild("GNX_S")
    RB_ZFKLF = RB_EventsFolder:FindFirstChild("ZFKLF__H")
    return RB_GNX ~= nil and RB_ZFKLF ~= nil
end

local function RB_RandomKey(len)
    local s = ""
    for _ = 1, len do s = s .. string.char(math.random(97, 122)) end
    return s .. "0"
end

-- Получаем таргета: если задан ник — ищем его, иначе ближайший враг
local function RB_GetTarget()
    local myChar = LocalPlayer.Character
    local myHRP  = myChar and myChar:FindFirstChild("HumanoidRootPart")
    if not myHRP then return nil end

    -- Конкретный таргет по нику
    if RB.TargetName and RB.TargetName ~= "" then
        local p = Players:FindFirstChild(RB.TargetName)
        ...
        -- Если таргет задан но не найден/мёртв — не стреляем
        return nil
    end

    -- Автовыбор: ближайший живой враг
    local closest, bestDist = nil, RB.MaxDist
    for _, player in ipairs(Players:GetPlayers()) do
        if player == LocalPlayer then continue end
        local char = player.Character
        ...
        if hrp and hum and hum.Health > 0 and not char:FindFirstChildOfClass("ForceField") then
            ...
            if dist < bestDist then bestDist = dist; closest = player end
        end
    end
    return closest
end

-- Телепорт к таргету (если слишком далеко)
local function RB_TeleportToTarget(targetPlayer)
    ...
    if dist > RB.TeleportRange then
        -- Телепортируемся чуть позади таргета чтобы не стоять в нём
        local offset = (myHRP.Position - tHRP.Position).Unit * (RB.TeleportRange * 0.5)
        myHRP.CFrame = CFrame.new(tHRP.Position + offset + Vector3.new(0, 2, 0))
    end
end

-- Выстрел в конкретного таргета, пули летят именно в него
local function RB_Shoot(targetPlayer)
    if not targetPlayer or not targetPlayer.Character then return end
    if not RB_EnsureRemotes() then
        warn("RageBot: ремоты не найдены")
        return
    end

    -- Всегда целимся в голову
    ...
    local cam        = Workspace.CurrentCamera
    local hitPos     = aimPart.Position
    local hitDir     = (hitPos - cam.CFrame.Position).Unit
    local key        = RB_RandomKey(30)

    -- FireServer GNX_S — регистрируем выстрел
    pcall(function()
        RB_GNX:FireServer(
            tick(),
            key,
    ...
    -- FireServer ZFKLF__H — говорим серверу в кого попали (именно наш таргет)
    pcall(function()
        RB_ZFKLF:FireServer(
            "\xF0\x9F\xA7\x88",
            tool,
...
local RB_Running   = false
local RB_Coroutine = nil

local function RB_Loop()
    while RB.Enabled do
        local target = RB_GetTarget()
        if target then
            -- Телепортируемся к нему если нужно
            RB_TeleportToTarget(target)
            -- Стреляем
            RB_Shoot(target)
            task.wait()   -- максимальная скорость (один кадр)
        else
            task.wait(0.1)
        end
    end
...
local function RB_Enable()
    if not RB_EnsureRemotes() then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "RageBot", Text = "Ремоты GNX_S / ZFKLF__H не найдены!", Duration = 5
            })
        end)
        return
    end
    if RB.Enabled then return end
    RB.Enabled = true
    RB_Running  = true
    RB_Coroutine = coroutine.create(RB_Loop)
    coroutine.resume(RB_Coroutine)
end

local function RB_Disable()
    RB.Enabled  = false
    RB_Running  = false
end
...
do
    ...
    R_Tab:CreateToggle({
        ...
        Flag         = "RB_On",
        Callback     = function(v)
            if v then RB_Enable() else RB_Disable() end
        end,
    })

    -- Поле ника таргета
    R_Tab:CreateInput({
        Name        = "Target Nick (пусто = ближайший)",
    ...
    -- Макс дистанция (для режима ближайший)
    R_Tab:CreateSlider({
        Name         = "Max Distance (для авто-таргета)",
        Range        = { 10, 1000 },
        Increment    = 10,
        Suffix       = " studs",
        CurrentValue = 200,
    ...
    -- Дистанция телепорта
    R_Tab:CreateSlider({
        Name         = "Teleport Range (тп если дальше)",
        Range        = { 5, 200 },
        Increment    = 5,
        Suffix       = " studs",
        CurrentValue = 10,
        Flag         = "RB_TeleportRange",
        Callback     = function(v)
            RB.TeleportRange = v
        end,
    })
...
-- ──────── Infinite Stamina (из бабушкасвэг) ────────
...
-- ──────── Auto Unlock / Auto Open Doors (из бабушкасвэг) ────────
...
local function StartStopDoorLoop()
    if (OpenNearbyDoors_Enabled or UnlockNearbyDoors_Enabled) and not NearbyDoorInteraction_Coroutine then
...
local function OpenNearbyDoors_Enable()
    ...
    OpenNearbyDoors_Enabled = true; StartStopDoorLoop()
end
local function OpenNearbyDoors_Disable()
    if not OpenNearbyDoors_Enabled then return end
    OpenNearbyDoors_Enabled = false; StartStopDoorLoop()
end
local function UnlockNearbyDoors_Enable()
    if UnlockNearbyDoors_Enabled then return end
    UnlockNearbyDoors_Enabled = true; StartStopDoorLoop()
end
local function UnlockNearbyDoors_Disable()
    if not UnlockNearbyDoors_Enabled then return end
    UnlockNearbyDoors_Enabled = false; StartStopDoorLoop()
end

-- ──────── Safe ESP (BredMakurz из бабушкасвэг) ────────
local SafeESP_Enabled     = false
local SafeESP_Connection  = nil
local safeTimers          = {}

local function formatSafeName(name)
...
local function SafeESP_Update(delta)
    local mapFolder = Workspace:FindFirstChild("Map")
    local bredMakurzFolder = mapFolder and mapFolder:FindFirstChild("BredMakurz")
    ...
    for _, v in pairs(bredMakurzFolder:GetChildren()) do
        ...
        -- Убираем старый хайлайт если остался
        if v:FindFirstChild("EspHighlight") then v.EspHighlight:Destroy() end

        local existingGui = v:FindFirstChild("TereyakiSafeESP")

        if isBroken then
            if not safeTimers[v.Name] then safeTimers[v.Name] = 720 end
            if safeTimers[v.Name] > 0 then safeTimers[v.Name] = safeTimers[v.Name] - delta end
            ...
            if existingGui then
                ...
                if label then
                    ...
                    label.Text       = string.format("%s\n[COOLDOWN]\n%02d:%02d", formatSafeName(v.Name), minutes, seconds)
            ...
            if not existingGui then
                ...
                existingGui.Name = "TereyakiSafeESP"
                existingGui.AlwaysOnTop  = true
                existingGui.Size         = UDim2.new(0, 120, 0, 60)
                existingGui.MaxDistance  = math.huge
                existingGui.StudsOffset  = Vector3.new(0, 4, 0)

                local lbl = Instance.new("TextLabel", existingGui)
            ...
            local label = existingGui:FindFirstChildOfClass("TextLabel")
            if label then
                label.Text       = formatSafeName(v.Name) .. "\n[READY]"
...
local function SafeESP_Enable()
    if SafeESP_Enabled then return end
    SafeESP_Enabled = true
    local lastTime  = tick()
    SafeESP_Connection = RunService.Heartbeat:Connect(function()
        local now   = tick()
        local delta = now - lastTime
        lastTime    = now
        SafeESP_Update(delta)
    end)
end

local function SafeESP_Disable()
    if not SafeESP_Enabled then return end
    SafeESP_Enabled = false
    if SafeESP_Connection then SafeESP_Connection:Disconnect(); SafeESP_Connection = nil end
    safeTimers = {}
    local mapFolder = Workspace:FindFirstChild("Map")
    local bredMakurzFolder = mapFolder and mapFolder:FindFirstChild("BredMakurz")
    if bredMakurzFolder then
        for _, v in pairs(bredMakurzFolder:GetChildren()) do
            if v:FindFirstChild("TereyakiSafeESP") then v.TereyakiSafeESP:Destroy() end
        end
    end
end

-- ──────── Noclip (standalone, независимый от автофарма) ────────
local Noclip_Enabled    = false
local Noclip_Connection = nil
local Noclip_OriginalCollisions = {}

local function Noclip_Enable()
    if Noclip_Enabled then return end
    Noclip_Enabled = true
    Noclip_OriginalCollisions = {}
    local char = LocalPlayer.Character
    if char then
        for _, part in pairs(char:GetDescendants()) do
            if part:IsA("BasePart") and part.CanCollide then
                Noclip_OriginalCollisions[part] = true
                part.CanCollide = false
            end
        end
    end
...
local function Noclip_Disable()
    ...
    if char then
        for _, part in pairs(char:GetDescendants()) do
            if part:IsA("BasePart") and Noclip_OriginalCollisions[part] then
...
do
    ...
    M_Tab:CreateToggle({
        ...
        Callback     = function(v)
            if v then InfStamina_Enable() else InfStamina_Disable() end
        end,
    })
    ...
    M_Tab:CreateToggle({
        ...
        CurrentValue = false,
        Flag         = "MISC_UnlockDoors",
        Callback     = function(v)
            if v then UnlockNearbyDoors_Enable() else UnlockNearbyDoors_Disable() end
        end,
    })
    ...
    M_Tab:CreateToggle({
        ...
        Flag         = "MISC_OpenDoors",
        Callback     = function(v)
            if v then OpenNearbyDoors_Enable() else OpenNearbyDoors_Disable() end
        end,
    })
    ...
    M_Tab:CreateToggle({
        ...
        Flag         = "MISC_Noclip",
        Callback     = function(v)
            if v then Noclip_Enable() else Noclip_Disable() end
        end,
    })

    M_Tab:CreateToggle({
        Name         = "Safe ESP (Ready / Cooldown)",
        CurrentValue = false,
        Flag         = "MISC_SafeESP",
        Callback     = function(v)
            if v then SafeESP_Enable() else SafeESP_Disable() end
        end,
    })
end
...
-- ============================================================
do
    local MenuGroup = TabUISet:AddLeftGroupbox("Menu", "wrench")
    ...
    ThemeManager:ApplyToGroupbox(TabUISet:AddRightGroupbox("Theme"))
