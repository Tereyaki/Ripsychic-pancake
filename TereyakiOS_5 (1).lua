...
local SA = {
    ...
    DrawCircle   = false,
    DrawSize     = 150,
    VisCheck     = true,   -- wallcheck: пропускаем цели за стенами
    MaxDist      = 500,
    UseFOV       = true,
    IgnoreDowned = true,
    FOVCol       = Color3.fromRGB(70, 130, 255),

    -- Шанс попадания по каждой части тела (0 = часть отключена)
    PartChance = {
        Head       = 85,
        UpperTorso = 70,
        LowerTorso = 60,
    ...
    -- Псевдонимы костей для R6 и R15
    PartAliases = {
        Head       = {"Head"},
...
-- FOV-круг Silent Aim (следует за мышью)
...
-- Выбор части тела по шансу попадания
...
-- Выбор цели: ближайший к центру экрана (FOV-режим) / к игроку
-- + wallcheck из бабушкасвэга (луч от камеры к цели)
local function SA_GetValidTarget()
    if not SA.On then return nil end
    local Camera    = Workspace.CurrentCamera
    local target    = nil
    local minDist   = math.huge
    -- FOV считаем от мыши (как в бабушкасвэге)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        -- Wallcheck (из бабушкасвэга): луч от камеры к голове/HRP
        if SA.VisCheck then
        ...
        if SA.UseFOV then
            -- Считаем ближайший лимб к позиции мыши (как в бабушкасвэге)
        ...
        if viz then
            local conn = viz.Event:Connect(function(_, sc, _, gun, _, sPos, bps)
                ...
                task.spawn(function()
                    for i = 1, #nd do
                        task.wait(0.01)
                        pcall(function() ZFK_remote:FireServer("\xF0\x9F\xA7\x88", gun, sc, i, tp, hp, nd[i]) end)
                    end
                end)
                if gun:FindFirstChild("Hitmarker") then
                    pcall(function() gun.Hitmarker:Fire(tp) end)
                end
            end)
...
local function SA_Enable()
    ...
    -- Обновляем FOV-круг за мышью каждый кадр
...
do
    ...
    SA_Tab:CreateToggle({
        ...
        Flag         = "SA_On",
        Callback     = function(v)
            if v then SA_Enable() else SA_Disable() end
        end,
    })
    ...
    SA_Tab:CreateToggle({
        ...
        CurrentValue = true,
        Flag         = "SA_IgnoreDowned",
        Callback     = function(v) SA.IgnoreDowned = v end,
    })

    -- Шанс попадания по частям тела
    local partDefs = {
