...
local Tabs = {
    ...
    SilentAim = Window:CreateTab("Silent Aim"),
...
local function RB_Shoot(targetPlayer)
    ...
    local aimPart   = char:FindFirstChild(RB.AimPart)
...
do
    ...
    -- Часть тела
    R_Tab:CreateDropdown({
        Name          = "Aim Part",
        Options       = { "Head", "UpperTorso", "Torso", "HumanoidRootPart", "RightUpperArm", "LeftUpperArm" },
        CurrentOption = { "Head" },
        MultipleOptions = false,
        Flag          = "RB_AimPart",
        Callback      = function(v)
            if type(v) == "table" then v = v[1] end
            RB.AimPart = v or "Head"
        end,
    })

    -- Скорострельность
    R_Tab:CreateSlider({
        Name         = "Fire Rate (задержка сек ×0.01)",
        Range        = { 1, 50 },
        Increment    = 1,
        Suffix       = " ×0.01s",
        CurrentValue = 5,
